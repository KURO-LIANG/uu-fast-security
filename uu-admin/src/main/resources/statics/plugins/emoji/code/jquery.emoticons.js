...
;(function (factory) {
    ...
    $.emoticons = function(parameter,getApi) {
        ...
        var defaults = {
            ...
            'triggerCls':'trigger',
            'activeCls':'active',
            'path': baseURL + 'statics/plugins/emoji/public/image/',
            'list':[
                {'title':'微笑','url':'weixiao.gif'},
                {'title':'呲牙','url':'xixi.gif'},
                {'title':'憨笑','url':'haha.gif'},
                {'title':'愉快','url':'keai.gif'},
                {'title':'可怜','url':'kelian.gif'},
                {'title':'挖鼻','url':'wabi.gif'},
                {'title':'惊讶','url':'chijing.gif'},
                {'title':'害羞','url':'haixiu.gif'},
                {'title':'调皮','url':'jiyan.gif'},
        ...
        _api.format = function(str){
            ...
            if(list){
                for(var i=0;i<list.length;i++){
                    ...
                    if(_hash[title]){
                        str = str.replace(list[i],' <img data-emoji="['+title+']" src="'+_hash[title]+'"/> ');
                    }
                }
            }
            return str;
