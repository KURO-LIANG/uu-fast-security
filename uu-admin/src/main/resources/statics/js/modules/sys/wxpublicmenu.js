        ...
        this.$nextTick(function () {
            ...
            if( $("#menuList li").length >1){
                ...
                if(curType == 'TYPE_view'){ //跳转页面
                    ...
                    $("#url").css('display','none');
                    $("#sendDiy").css('display','none');
                }else if(curType == 'TYPE_' || curType == 'TYPE_undefined'){ //类型为空 有子菜单
    ...
    if(type == 'TYPE_view'){ //跳转页面
        ...
        $.get(baseURL + "sys/wxpublicmenu/getMediaByID?mid="+mid, function(r){
            if(r.code === 0){
                ...
                $(".send").find('.layui-tab-item').eq(0).find('.jsMsgSendTab').css('display','none');
                $(".send").find('.layui-tab-item').eq(0).find('.appmsgLeft').remove();
                $(".send").find('.layui-tab-item').eq(0).find('.inner').append(mediaHtml);
            }else{
...
$("#jsDelBt").on('click',function () {
    ...
    if(length <=0){
        ...
        layer.confirm('确定要删除该菜单？',function () {
            ...
            var subLen = $("#menuList").find(".current").parent().find('li').length;
            console.log("子菜单删除前总长度：" + subLen);

            if(subLen === 6){
                $("#menuList").find(".current").parent().find('li').eq(-1).css('display','block');
            }
...
$(".pubBt").click(function (e) {
    ...
    for (var i=0;i< len;i++){
        ...
        }else if(typeCheck == 'TYPE_' || typeCheck == 'TYPE_undefined'){ //类型为空
    ...
    for (var i=0;i< menuLen;i++){
        ...
        }else if(type == 'TYPE_' || type == 'TYPE_undefined'){
