$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'orderremark/orderremark/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.orderRemark.id == null ? "orderremark/orderremark/save" : "orderremark/orderremark/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.orderRemark),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "orderremark/orderremark/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "orderremark/orderremark/info/"+id, function(r){
                vm.orderRemark = r.orderRemark;
            });
		},
