	...
	$.fn.extend({
		takungaeImgup : function(opt, serverCallBack) {
			...
			$(this).change(function() {
				...
				if (fileList.length > imageNum|| totalNum > imageNum) {
					...
					for ( var i = 0; i < fileList.length; i++) {
						...
						var $span = $("<span class='up-span imgIndex'>");
						...
						$img0.attr("src",baseURL + "//statics/img/a7.png").appendTo($section);
						var $img = $("<img class='up-img up-opcity'>");
						// $img.attr("src", imgArr[i]); //图片路径
						$img.appendTo($section);
						var $p = $("<p class='img-name-p'>");
						$p.html(fileList[i].name).appendTo($section);
						var $input = $("<input class='prod_img' name='prod_img' value='' type='hidden'>");
						$input.appendTo($section);
						var $input2 = $("<input class='ban_img' name='ban_img' value='' type='hidden'/>");
						$input2.appendTo($section);
						uploadImg(opt, fileList[i],$section,$img); //发请求传到服务器
			...
			$(".z_photo").delegate(".close-upimg", "click", function(event) {
				...
				$(".works-mask").show();
				delParent = $(this).parent();
				console.log(delParent.html()+"删除图片了=======");
			...
			function uploadImg(opt, file, obj,$img) {
				...
				$.ajax({
					...
					success : function(data) {
						$img.attr("src",data.result.fileUrl); //图片路径
					},
					error : function(e) {
						obj.remove();
						var err = "上传失败，请联系管理员！";
