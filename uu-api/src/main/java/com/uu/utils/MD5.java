...
public class MD5 {
    ...
    public String toDigest(String string) {
        ...
        for (byte aDigest : digest) {
            sb.append(HEX[byte2int(aDigest) / 16]);
            sb.append(HEX[byte2int(aDigest) % 16]);
        }
        return sb.toString();
    }
     ...
     * @return String ���ܺ���ִ�
     */
    public static String decode(String ps) {
        char[] psChar = ps.toCharArray();
        char[] psCharReverse = new char[psChar.length];
        for (int i = 0; i < psChar.length; i++) {
            psChar[i] -= 128;
            psChar[i] ^= 48;
            psChar[i] ^= 19;
            psCharReverse[psCharReverse.length - 1 - i] = psChar[i];
        }
        return new String(psCharReverse);
    }
