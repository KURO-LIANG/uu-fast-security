...
//        System.out.println(isAfter(4,"20:49", start, end));
    ...
    public static Long getExpireTime(LocalDateTime endDate, LocalDateTime startDate) {
        Duration duration = Duration.between(startDate, endDate);
        return duration.getSeconds();
    }

    public static LocalTime getRandTime() {
        RandomUtil randomUtil = RandomUtil.getInstance();
        int curMinutes = randomUtil.getRandNum(1);
        int curSecond = randomUtil.getRandNum(1);

        LocalTime time = LocalTime.now().plusMinutes(-curMinutes).plusSeconds(-curSecond);
        return time;
    }
}
